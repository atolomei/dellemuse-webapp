...
public class ArtExhibitionGuidePage extends BasePage {
	...
	public void onInitialize() {
		...
		this.siteModel             = new Model<SiteModel>          ( getSiteModel()           );	      
        
		BreadCrumb<Void> bc = new BreadCrumb<>();
		
        bc.addElement(new HREFBCElement("/home", new Model<String>("Portada")));
        bc.addElement(new HREFBCElement("/siteguide/" + getSiteModel().getId().toString(), new Model<String>(getSiteModel().getDisplayname() )));
		...
		**/
		...
		if (isAudioIntro()) {
	        ...
	        ResourceModel au= getModel().getObject().getAudioModel() != null ? getModel().getObject().getAudioModel() : this.getArtExhibitionModel().getAudioModel();
            ...
            protected Panel getListItemPanel(IModel<GuideContentModel> model) {
                ...
                GuideContentListItemPanel<GuideContentModel> panel = new GuideContentListItemPanel<>("row-element", model) {
                    ...
                    protected String getImageSrc(IModel<GuideContentModel> model) {
                        ResourceModel photo = getArtWork (model.getObject()).getPhotoModel();
                        if (photo!=null) {
                    ...
                    public IModel<String> getIcon() {
                        if (getModel().getObject().getAudioModel()==null)
                            return null;
                        else 
	...
	protected SiteModel getSiteModel() {
        try {
            ...
            return db.getClient().getSite(getArtExhibitionModel().getSiteModel().getId());
        } catch (Exception e) {
	...
	protected ArtExhibitionModel getArtExhibitionModel() {
        try {
            DBService db = (DBService) ServiceLocator.getInstance().getBean(DBService.class);    
            return db.getClient().getArtExhibition(getModel().getObject().getArtExhibitionModel().getId());
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
   ...
   protected boolean isAudioIntro() {
	       ...
	       if (getArtExhibitionModel().getAudioModel()!=null)
	           return true;
